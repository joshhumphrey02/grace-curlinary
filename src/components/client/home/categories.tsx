	...
	return (
		<>
			<div className="space-y-4 my-6 w-full ">
					...
					{data.map((c, i) => (
						<Link key={uniqueId()} href={'#'}>
							<Card className="p-1 space-y-4 outline-tertiary transition-all duration-1000 hover:outline rounded-xl shadow-lg transform hover:scale-105 hover:shadow-2xl">
								...
								<div>
									<h5 className="text-xs font-bold text-center text-gray-400 text-nowrap px-1 pb-2">
										{c.name}
									</h5>
								</div>
							</Card>
				...
				<div className="flex max-w-[22rem] sm:hidden whitespace-nowrap space-x-4 overflow-x-auto w-full scroll-smooth">
					{[...data, ...data].map((c, i) => (
						<Link key={uniqueId()} href={'#'}>
